...
//  Iron
//
//  Created by Shaun Campbell on 2016-10-14.
...
class AppDelegate: UIResponder, UIApplicationDelegate {
    ...
    func application(_ application: UIApplication, didFinishLaunchingWithOptions launchOptions: [UIApplicationLaunchOptionsKey: Any]?) -> Bool {
        ...
        let launchStoryboard = UIStoryboard.init(name: "LaunchScreen", bundle: Bundle.main)
        ...
        FIRAuth.auth()?.addStateDidChangeListener({ (auth, user) in
            if user != nil {
                UserManager.shared.logInCurrentUser()
...
//                self.window?.rootViewController = homeStoryboard.instantiateInitialViewController()
                ...
                self.window?.rootViewController = SlideMenuController(mainViewController: (homeStoryboard.instantiateInitialViewController())!, leftMenuViewController: homeStoryboard.instantiateViewController(withIdentifier: "MenuTableView"))
